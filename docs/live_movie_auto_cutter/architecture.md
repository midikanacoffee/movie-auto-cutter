...
### ADR-07: 4区分セグメンテーション（Opening/Song/MC/Ending）と動画演出（フェード・テロップ合成）
* **背景**:
  1. ライブ動画では「開場〜1曲目までのオープニング」や「最後の曲の演奏終了から退場・終演アナウンスまでのエンディング」もファンにとって価値ある記録である。
  2. YouTube投稿や鑑賞用として、曲頭・曲末のフェードイン/アウト、曲名・アーティスト名テロップ、および「ご視聴ありがとうございました」などの終了メッセージを付けたいという要望がある。
  3. 歌詞の聞き取りはAIの幻覚（Hallucination）により不正確になりやすいため、無理な推測を抑制する必要がある。
* **決定**:
  - **4区分認識**: Geminiのプロンプトを改訂し、動画全体を `opening`, `song`, `mc`, `ending` の4区分で構造化抽出。オープニングは `00_[Opening]_...mp4`、エンディングは `99_[Ending]_...mp4` として独立保存可能にする。
  - **動画演出エンジン (`src/video_effects.py`)**: ffmpegの `fade`（映像・音声）および `drawtext` フィルタを動的に構築。Windows環境に標準搭載されている日本語フォント（Meiryo）を自動検出し、文字化けや描画エラーを防止。四隅（左下・右下・左上・右上）の位置指定に対応。
  - **ハイブリッドエンコード設計**: 演出が指定されていない場合は超高速なストリームコピー（`-c copy`）で瞬時に切り出し、演出が指定された場合のみ ffmpeg のフィルタグラフを通して再エンコード（`libx264` + `aac`）を実行。
  - **歌詞幻覚の抑制**: プロンプトで「はっきり聞き取れる場合のみ記述し、推測や創作は厳禁。聞き取れない場合は空文字」と明示。
* **効果**: ライブ全体の完全なアーカイブ化と、YouTube投稿に即座に耐えうるプロ品質の動画演出が自動で得られる。
