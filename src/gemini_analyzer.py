...
3. **歌詞（lyrics）の書き起こし**:
   - 各曲について、聞き取れた歌詞（特にサビや印象的なフレーズ）を数行程度書き起こしてください。
4. **YouTube投稿用メタデータ（youtube_metadata）の作成**:
   - YouTubeにそのまま動画をアップロードできるよう、以下のリッチな情報を各曲ごとに生成してください：
     - title: 【Live】曲名 - アーティスト名 (日付等)
     - description: 視聴者の興味を引く魅力的な概要欄説明文（曲の紹介、演奏のハイライト、歌詞の抜粋など）
     - mood_and_atmosphere: 演奏や会場の雰囲気（例: 「力強いドラムと熱狂的なコール＆レスポンス」「しっとりとしたアコースティックバラード」など）
     - recorded_date: MCでの発言（「今日は〇月〇日〜」など）やファイル名から推測されるライブ開催日時
     - tags: おすすめのYouTubeタグ・ハッシュタグ（5〜10個）
5. **セグメントの種類（segment_type）**:
   - "song": 楽曲演奏
   - "mc": トーク・メンバー紹介・MC
